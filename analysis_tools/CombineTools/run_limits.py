...
def run_combine(signal_dir, signals, max_jobs):
    ...
    for signal in signals:
        ...
        command = "nice -n {} combine {}/datacard_{}.txt -M AsymptoticLimits -n {} --run blind --cminDefaultMinimizerStrategy 0".format(10, signal_dir, signal, signal) # --cminDefaultMinimizerTolerance 0
        # command = "nice -n {} combine {}/datacard_{}.txt -M MarkovChainMC -t 10 --toysFrequentist --bypassFrequentistFit --cminDefaultMinimizerStrategy 0 --noDefaultPrior=0 -n {}".format(10, signal_dir, signal, signal) # --cminDefaultMinimizerTolerance 0
        # command = "nice -n {} combine {}/datacard_{}.txt -M HybridNew --LHCmode LHC-limits --rMin -2 --rMax 2 -n {} --cminDefaultMinimizerStrategy 0".format(10, signal_dir, signal, signal)
...
def get_limits(signal_dir, signals, b_blind):
    ...
    keys = {"0.025":"low_95", "0.16":"low_68", "0.5":"central", "0.84":"high_68", "0.975":"high_95", "-1":"observed"}
    for signal in signals:
        ...
        for entry in t:
            # print n
            # print entry.limit
            # print str(round(entry.quantileExpected,3))
            values[keys[str(round(entry.quantileExpected,3))]].append(entry.limit)
        ...
        for key, value in keys.iteritems():
            if values[value]:
                limits[value].append(sum(values[value]) / len(values[value]))
    return limits
