...
def draw_syst_shapes(fname, backgrounds, channels, systematics, syst_map):
    ...
    for channel in channels:
        h_nominal = fin.Get(channel+"__"+backgrounds[0]).Clone("nominal")
        h_nominal.Reset()
        for process in backgrounds:
            h_nominal.Add(fin.Get(channel+"__"+process))
        h_nominal.SetFillColor(796) # kOrange-4

        for syst in systematics:
            h_nominal.SetTitle(channel+"_"+syst)
            h_syst_up = h_nominal.Clone(channel+"_"+syst+"_up") 
            h_syst_down = h_nominal.Clone(channel+"_"+syst+"_down")
            h_syst_up.Reset()
            h_syst_down.Reset()
            ...
            for process in backgrounds:
                if syst in syst_map[channel][process]:
                    ...
                    h_syst_down.Add(fin.Get(channel+"__"+process))
