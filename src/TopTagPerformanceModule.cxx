...
namespace uhh2 {
  ...
  TopTagPerformanceModule::TopTagPerformanceModule(Context & ctx) {
    ...
    TopJetId id_wreco  = AndId<TopJet>(id_topjet, DeltaPhiCut(deltaPhi_min));
    ...
    hotvr_jetlep_cleaner.reset(new HOTVRLeptonCleaner(0.4));
  ...
  bool TopTagPerformanceModule::process(Event & event) {
   ...
   if(!is_mc)
    ...
    // hotvr_jetlep_cleaner->process(event);
    if(is_mc)
      {
