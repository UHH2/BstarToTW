...
BstarToTWHypothesisHists::BstarToTWHypothesisHists(uhh2::Context & ctx, const std::string & dirname, const std::string & hyps_name, const std::string & discriminator_name ): Hists(ctx, dirname){
  ...
  TString name = discriminator_name;
    ...
    Bstar_reco_M = book<TH1F>("Bstar_reco_M", "M_{b*}^{reco} [GeV/c^{2}]", 100, 0, 5000);
    ...
    Top_reco_M = book<TH1F>("Top_reco_M", "M_{top}^{reco} [GeV/c^{2}]", 70, 0, 700);
    Top_reco_Pt = book<TH1F>("Top_reco_Pt", "p_{T, top}^{reco} [GeV/c]", 100, 0, 2000);
    DeltaR_top_W = book<TH1F>("DeltaR_top_W", "#Delta R_{top,W}", 60, 0, 6);
    DeltaPhi_top_W = book<TH1F>("DeltaPhi_top_W", "#Delta #phi_{top,W}", 65, 0, 6.5);
    DeltaPt_top_W = book<TH1F>("DeltaPt_top_W", "#Delta p_{T, top,W} [GeV/c]", 80, -400, 400);
    DeltaPt_top_W_over_pt = book<TH1F>("DeltaPt_top_W_over_pt", "#Delta p_{T top,W}/p_{T top}", 40, -1, 1);
...
void BstarToTWHypothesisHists::fill(const uhh2::Event & e){
  ...
  if (!hyp)
    {
      cout << "WARNING: No hypothesis was valid!" << endl;
      return;
    }
  ...
  DeltaPhi_top_W->Fill(delPhi, weight);
