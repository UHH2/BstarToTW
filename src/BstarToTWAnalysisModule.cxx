...
namespace uhh2 {
  ...
  private:  
    ...
    bool is_ele, is_muo, do_for_analysis;
    ...
    std::unique_ptr<AnalysisModule> primary_lep, hadronic_top;
    ...
    std::unique_ptr<AndHists> hist_sel_1btagdphi, hist_sel_2btagdphi, hist_sel_0btagdphi;
    ...
    // - jet deltaPhi cut - //
    std::unique_ptr<Selection> sel_jet_deltaphi, sel_bjet_deltaphi;
  ...
  BstarToTWAnalysisModule::BstarToTWAnalysisModule(Context & ctx) {
    ...
    do_for_analysis = ctx.get("do_for_analysis") == "true";
    ...
    // --- Object IDs --- //
    TopJetId id_topjet = VetoId<TopJet>(DeltaPhiCut(ctx, deltaPhi_min));
    // Toptags:
    ...
    TopJetId id_antitoptag = AndId<TopJet>(HOTVRInvertedMassCut(top_fpt_max, top_m_min, top_m_max, top_mpair_min), VetoId<TopJet>(Tau32Groomed(top_tau32_max)));
    ...
    string sys_toptag = ctx.get("HOTVRTopTagSF_direction","nominal");
    sf_toptag.reset(new HOTVRScaleFactor(ctx, id_toptag, sys_toptag));
    ...
    reco_0toptag.reset(new BstarToTWReconstruction(ctx, NeutrinoReconstruction, "0TopTagReconstruction", id_antitoptag));
    ...
    // --- Selections and Histogramms --- //
    sel_1top.reset(new NTopJetSelection(0, 0, id_topjet));
    ...
    // - deltaPhi jet - //
    sel_jet_deltaphi.reset(new JetDeltaPhiSelection(ctx, deltaPhi_min));
    sel_bjet_deltaphi.reset(new JetDeltaPhiSelection(ctx, deltaPhi_min, ctx.get_handle<vector<Jet> >("btag_loose")));
    ...
    hist_sel_1btagdphi.reset(new AndHists(ctx, "1btagdphi"));
    hist_sel_0btagdphi.reset(new AndHists(ctx, "0btagdphi"));
  ...
  bool BstarToTWAnalysisModule::process(Event & event) {
	...
	if (!sel_jet_deltaphi->passes(event)) return false;
	hist_sel_0btagdphi->fill(event);
	...
	else if (veto_toptag->passes(event) && sel_1antitoptag->passes(event))
	  {
	...
	if (!sel_bjet_deltaphi->passes(event)) return false;
	hist_sel_1btagdphi->fill(event);
	// - Signal Region - //
	if(sel_1toptag->passes(event))
	  {	    
